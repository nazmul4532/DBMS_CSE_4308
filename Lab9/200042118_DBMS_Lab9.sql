...
-- TASK 2 D
CREATE OR REPLACE
PROCEDURE MOVIE_CATEGORY(MOVIE_TITLE IN VARCHAR2)
AS
YEAR VARCHAR2(20);
CURSOR MOV_INFO IS
    SELECT MOV_ID,MOV_TITLE, MAX(MOV_RELEASEDATE) AS MOV_RELEASEDATE, AVG(REV_STARS) AS AVG_RATING
    FROM MOVIE NATURAL JOIN RATING
    WHERE MOV_TITLE = MOVIE_TITLE
    GROUP BY MOV_ID,MOV_TITLE
    ORDER BY AVG_RATING DESC;
BEGIN
    FOR i IN MOV_INFO LOOP
        YEAR := TO_CHAR(i.MOV_RELEASEDATE, 'YYYY');
        IF i.AVG_RATING>6.5 AND YEAR>=1950 AND YEAR<=1959 THEN
            DBMS_OUTPUT.PUT_LINE('The Movie is From the Fantastic Fifties');
        ELSIF i.AVG_RATING>6.7 AND YEAR>=1960 AND YEAR<=1969 THEN
            DBMS_OUTPUT.PUT_LINE('The Movie is From the Sweet Sixties');
        ELSIF i.AVG_RATING>6.9 AND YEAR>=1970 AND YEAR<=1979 THEN
            DBMS_OUTPUT.PUT_LINE('The Movie is From the Super Seventies');
        ELSIF i.AVG_RATING>7.1 AND YEAR>=1980 AND YEAR<=1989 THEN
            DBMS_OUTPUT.PUT_LINE('The Movie is From the Ecstatic Eighties');
        ELSIF i.AVG_RATING>7.3 AND YEAR>=1990 AND YEAR<=1999 THEN
            DBMS_OUTPUT.PUT_LINE('The Movie is From the Neat Nineties');
        ELSE
            DBMS_OUTPUT.PUT_LINE('The Movie is Garbage');
        END IF;
    END LOOP;
END;
/
